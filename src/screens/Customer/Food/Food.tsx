...
const FoodScreen = () => {
  ...
  const { data: foodData, loading: foodLoading } = useQuery<GetFoodsData>(
    GET_FOODS,
    {
  ...
  const { data: restData, loading: restLoading } = useQuery<GetRestaurantsData>(
    GET_RESTAURANTS,
    {
      variables: { category: category || 'All' },
      fetchPolicy: 'cache-and-network',
  ...
  const openRestaurantsData = (restData?.getRestaurants || []).map((r: any) => {
    ...
    if (r.image) {
      finalUri = r.image.startsWith('http')
        ? { uri: r.image }
    ...
    return {
      ...
      delivery:
        r.deliveryFee && r.deliveryFee > 0
          ? `${Number(r.deliveryFee).toLocaleString('vi-VN')} ₫`
          : 'Miễn phí',
      ...
      <View style={styles.headerBar}>
        ...
        <View style={styles.headerActions}>
          <TouchableOpacity
            style={[
              styles.circleBtn,
              { backgroundColor: '#181C2E', marginRight: 10 },
            ]}
            onPress={goToSearch}
          >
      ...
      <View style={styles.sectionContainer}>
        ...
        {restLoading ? (
          ...
          <Text style={styles.emptyText}>
            Không có quán ăn cho danh mục này.
          </Text>
        ) : (
          <FlatList
            horizontal
            ...
            renderItem={({ item }) => (
              ...
              >
                ...
                <Text style={styles.restaurantNameH} numberOfLines={1}>
                  {item.name}
                </Text>
    ...
    <View style={styles.container}>
      {foodLoading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <Text
              style={[styles.emptyText, { marginTop: 50, textAlign: 'center' }]}
            >
              No foods found.
            </Text>
          }
          renderItem={({ item }) => (
            ...
            >
              ...
              <Text style={styles.foodRest} numberOfLines={1}>
                {item.restaurant}
              </Text>
